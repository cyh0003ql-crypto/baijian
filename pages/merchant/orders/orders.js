...
Page({
  data: {
    orders: [], filterStatus: '',
    tabs: [
      { label: '全部', status: '' }, { label: '待发货', status: 'paid' },
      { label: '已发货', status: 'shipped' }, { label: '已完成', status: 'completed' },
    ],
    activeTab: 0,
  },
  onLoad() {
  ...
  onQuickShip(e) {
    const { id } = e.currentTarget.dataset;
    wx.showModal({
      title: '一键发货', content: '确认现在发货吗？物流单号可以稍后补充。',
      confirmText: '确认发货', confirmColor: '#C87941',
      success: (res) => {
        if (!res.confirm) return;
        db.updateOrderStatus(id, 'shipped', { carrier: '快递', number: '', shippedAt: new Date().toISOString() });
        this.loadOrders();
